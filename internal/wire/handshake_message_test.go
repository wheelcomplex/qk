package wire

import (
	"bytes"
	"strings"

	"github.com/lucas-clemente/quic-go/internal/protocol"
	"github.com/lucas-clemente/quic-go/qerr"
	. "github.com/onsi/ginkgo"
	. "github.com/onsi/gomega"
)

var _ = Describe("Handshake Message", func() {
	var sampleCHLO = []byte{0x43, 0x48, 0x4c, 0x4f, 0x10, 0x0, 0x0, 0x0, 0x50, 0x41, 0x44, 0x0, 0xf8, 0x3, 0x0, 0x0, 0x53, 0x4e, 0x49, 0x0, 0x7, 0x4, 0x0, 0x0, 0x56, 0x45, 0x52, 0x0, 0xb, 0x4, 0x0, 0x0, 0x43, 0x43, 0x53, 0x0, 0x1b, 0x4, 0x0, 0x0, 0x4d, 0x53, 0x50, 0x43, 0x1f, 0x4, 0x0, 0x0, 0x55, 0x41, 0x49, 0x44, 0x4c, 0x4, 0x0, 0x0, 0x54, 0x43, 0x49, 0x44, 0x50, 0x4, 0x0, 0x0, 0x50, 0x44, 0x4d, 0x44, 0x54, 0x4, 0x0, 0x0, 0x53, 0x52, 0x42, 0x46, 0x58, 0x4, 0x0, 0x0, 0x49, 0x43, 0x53, 0x4c, 0x5c, 0x4, 0x0, 0x0, 0x4e, 0x4f, 0x4e, 0x50, 0x7c, 0x4, 0x0, 0x0, 0x53, 0x43, 0x4c, 0x53, 0x80, 0x4, 0x0, 0x0, 0x43, 0x53, 0x43, 0x54, 0x80, 0x4, 0x0, 0x0, 0x43, 0x4f, 0x50, 0x54, 0x84, 0x4, 0x0, 0x0, 0x43, 0x46, 0x43, 0x57, 0x88, 0x4, 0x0, 0x0, 0x53, 0x46, 0x43, 0x57, 0x8c, 0x4, 0x0, 0x0, , 0x1, 0xe8, 0x81, 0x60, 0x92, 0x92, 0x1a, 0xe8, 0x64, 0x0, 0x0, 0x0, 0x64, 0x65, 0x76, 0x20, 0x43, 0x68, 0x72, 0x6f, 0x6d, 0x65, 0x2f, 0x35, 0x31, 0x2e, 0x30, 0x2e, 0x32, 0x37, 0x30, 0x30, 0x2e, 0x30, 0x20, 0x49, 0x6e, 0x74, 0x65, 0x6c, 0x20, 0x4d, 0x61, 0x63, 0x20, 0x4f, 0x53, 0x20, 0x58, 0x20, 0x31, 0x30, 0x5f, 0x31, 0x31, 0x5f, 0x34, 0x0, 0x0, 0x0, 0x0, 0x58, 0x35, 0x30, 0x39, 0x0, 0x0, 0x10, 0x0, 0x1e, 0x0, 0x0, 0x0, 0xe1, 0x84, 0x54, 0x1b, 0xe3, 0xd6, 0x7c, 0x1f, 0x69, 0xb2, 0x4e, 0x9e, 0x46, 0xf4, 0x46, 0xdd, 0xab, 0xe5, 0xde, 0x66, 0x94, 0xf6, 0xb2, 0xee, 0x1, 0xc4, 0xa5, 0x77, 0xfe, 0xc9, 0xb, 0xa3, 0x1, 0x0, 0x0, 0x0, 0x46, 0x49, 0x58, 0x44, 0x0, 0x0, 0xf0, 0x0, 0x0, 0x0, 0x60, 0x0}

	var sampleCHLOMap = map[protocol.Tag][]byte{
		protocol.TagPAD:  []byte(strings.Repeat("-", 1016)),
		protocol.TagSNI:  []byte("www.example.org"),
		protocol.TagVER:  []byte("Q030"),
		protocol.TagCCS:  []byte("{&\xe9\xe7\xe4\\q\xff\x01\xe8\x81`\x92\x92\x1a\xe8"),
		protocol.TagMSPC: []byte("d\x00\x00\x00"),
		protocol.TagUAID: []byte("dev Chrome/51.0.2700.0 Intel Mac OS X 10_11_4"),
		protocol.TagTCID: []byte("\x00\x00\x00\x00"),
		protocol.TagSRBF: []byte("\x00\x00\x10\x00"),
		protocol.TagICSL: []byte("\x1e\x00\x00\x00"),
		protocol.TagNONP: []byte("\xe1\x84T\x1b\xe3\xd6|\x1fi\xb2N\x9eF\xf4Fݫ\xe5\xdef\x94\xf6\xb2\xee\x01ĥw\xfe\xc9\v\xa3"),
		protocol.TagSCLS: []byte("\x01\x00\x00\x00"),
		protocol.TagCSCT: {},
		protocol.TagCOPT: []byte("FIXD"),
		protocol.TagSFCW: []byte("\x00\x00`\x00"),
		protocol.TagCFCW: []byte("\x00\x00\xf0\x00"),
		protocol.TagPDMD: []byte("X509"),
	}

	Context("when parsing", func() {
		It("parses sample CHLO message", func() {
			msg, err := ParseHandshakeMessage(bytes.NewReader(sampleCHLO))
			Expect(err).ToNot(HaveOccurred())
			Expect(msg.Tag).To(Equal(protocol.TagCHLO))
			Expect(msg.Data).To(Equal(sampleCHLOMap))
		})

		It("rejects large numbers of pairs", func() {
			r := bytes.NewReader([]byte("CHLO\xff\xff\xff\xff"))
			_, err := ParseHandshakeMessage(r)
			Expect(err).To(MatchError(qerr.CryptoTooManyEntries))
		})

		It("rejects too long values", func() {
			r := bytes.NewReader([]byte{
				'C', 'H', 'L', 'O',
				1, 0, 0, 0,
				0, 0, 0, 0,
				0xff, 0xff, 0xff, 0xff,
			})
			_, err := ParseHandshakeMessage(r)
			Expect(err).To(MatchError(qerr.Error(qerr.CryptoInvalidValueLength, "value too long")))
		})
	})

	Context("when writing", func() {
		It("writes sample message", func() {
			b := &bytes.Buffer{}
			HandshakeMessage{Tag: protocol.TagCHLO, Data: sampleCHLOMap}.Write(b)
			Expect(b.Bytes()).To(Equal(sampleCHLO))
		})
	})

	Context("string representation", func() {
		It("has a string representation", func() {
			str := HandshakeMessage{
				Tag: protocol.TagSHLO,
				Data: map[protocol.Tag][]byte{
					protocol.TagAEAD: []byte("foobar"),
					protocol.TagEXPY: []byte("raboof"),
				},
			}.String()
			Expect(str[:4]).To(Equal("SHLO"))
			Expect(str).To(ContainSubstring("AEAD: \"foobar\""))
			Expect(str).To(ContainSubstring("EXPY: \"raboof\""))
		})

		It("lists padding separately", func() {
			str := HandshakeMessage{
				Tag: protocol.TagSHLO,
				Data: map[protocol.Tag][]byte{
					protocol.TagPAD: bytes.Repeat([]byte{0}, 1337),
				},
			}.String()
			Expect(str).To(ContainSubstring("PAD"))
			Expect(str).To(ContainSubstring("1337 bytes"))
		})
	})
})
