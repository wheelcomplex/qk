...
func WritePublicReset(connectionID protocol.ConnectionID, rejectedPacketNumber protocol.PacketNumber, nonceProof uint64) []byte {
	...
	utils.LittleEndian.WriteUint32(b, uint32(protocol.TagPRST))
	utils.LittleEndian.WriteUint32(b, 2)
	utils.LittleEndian.WriteUint32(b, uint32(protocol.TagRNON))
	utils.LittleEndian.WriteUint32(b, 8)
	utils.LittleEndian.WriteUint32(b, uint32(protocol.TagRSEQ))
...
func ParsePublicReset(r *bytes.Reader) (*PublicReset, error) {
	pr := PublicReset{}
	msg, err := ParseHandshakeMessage(r)
	if err != nil {
		return nil, err
	}
	if msg.Tag != protocol.TagPRST {
	...
	if rseq, ok := msg.Data[protocol.TagRSEQ]; ok {
		if len(rseq) != 8 {
	...
	rnon, ok := msg.Data[protocol.TagRNON]
	if !ok {
