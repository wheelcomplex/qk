...
func getDefaultServerConfigClient() map[protocol.Tag][]byte {
	return map[protocol.Tag][]byte{
		protocol.TagSCID: bytes.Repeat([]byte{'F'}, 16),
		protocol.TagKEXS: []byte("C255"),
		protocol.TagAEAD: []byte("AESG"),
		protocol.TagPUBS: append([]byte{0x20, 0x00, 0x00}, bytes.Repeat([]byte{0}, 32)...),
		protocol.TagOBIT: bytes.Repeat([]byte{0}, 8),
		protocol.TagEXPY: {0x0, 0x6c, 0x57, 0x78, 0, 0, 0, 0}, // 2033-12-24
	}
}
...
var _ = Describe("Server Config", func() {
	...
	It("returns the parsed server config", func() {
		tagMap[protocol.TagSCID] = []byte{0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf}
		b := &bytes.Buffer{}
		wire.HandshakeMessage{Tag: protocol.TagSCFG, Data: tagMap}.Write(b)
		scfg, err := parseServerConfig(b.Bytes())
		Expect(err).ToNot(HaveOccurred())
		Expect(scfg.ID).To(Equal(tagMap[protocol.TagSCID]))
	})
	...
	Context("parsing the server config", func() {
		It("rejects a handshake message with the wrong message tag", func() {
			...
			wire.HandshakeMessage{Tag: protocol.TagCHLO, Data: make(map[protocol.Tag][]byte)}.Write(&serverConfig)
		...
		It("errors on invalid handshake messages", func() {
			var serverConfig bytes.Buffer
			wire.HandshakeMessage{Tag: protocol.TagSCFG, Data: make(map[protocol.Tag][]byte)}.Write(&serverConfig)
			_, err := parseServerConfig(serverConfig.Bytes()[:serverConfig.Len()-2])
			Expect(err).To(MatchError("unexpected EOF"))
		})

		It("passes on errors encountered when reading the protocol.TagMap", func() {
			var serverConfig bytes.Buffer
			wire.HandshakeMessage{Tag: protocol.TagSCFG, Data: make(map[protocol.Tag][]byte)}.Write(&serverConfig)
			_, err := parseServerConfig(serverConfig.Bytes())
		...
		It("reads an example Handshake Message", func() {
			var serverConfig bytes.Buffer
			wire.HandshakeMessage{Tag: protocol.TagSCFG, Data: tagMap}.Write(&serverConfig)
			scfg, err := parseServerConfig(serverConfig.Bytes())
			Expect(err).ToNot(HaveOccurred())
			Expect(scfg.ID).To(Equal(tagMap[protocol.TagSCID]))
			Expect(scfg.obit).To(Equal(tagMap[protocol.TagOBIT]))
		})
	})

	Context("Reading values from the protocol.TagMap", func() {
		...
		Context("ServerConfig ID", func() {
			...
			It("errors if the ServerConfig ID is missing", func() {
				delete(tagMap, protocol.TagSCID)
				err := scfg.parseValues(tagMap)
				Expect(err).To(MatchError("CryptoMessageParameterNotFound: SCID"))
			})

			It("rejects ServerConfig IDs that have the wrong length", func() {
				tagMap[protocol.TagSCID] = bytes.Repeat([]byte{'F'}, 17) // 1 byte too long
				err := scfg.parseValues(tagMap)
		...
		Context("KEXS", func() {
			...
			It("errors if the KEXS is missing", func() {
				delete(tagMap, protocol.TagKEXS)
				err := scfg.parseValues(tagMap)
		...
		Context("PUBS", func() {
			It("creates a Curve25519 key exchange", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				tagMap[protocol.TagPUBS] = append([]byte{0x20, 0x00, 0x00}, serverKex.PublicKey()...)
				err = scfg.parseValues(tagMap)
				Expect(err).ToNot(HaveOccurred())
			...
			It("rejects PUBS values that have the wrong length", func() {
				tagMap[protocol.TagPUBS] = bytes.Repeat([]byte{'F'}, 100) // completely wrong length
				err := scfg.parseValues(tagMap)
				Expect(err).To(MatchError("CryptoInvalidValueLength: PUBS"))
			})

			It("rejects PUBS values that have a zero length", func() {
				tagMap[protocol.TagPUBS] = bytes.Repeat([]byte{0}, 100) // completely wrong length
				err := scfg.parseValues(tagMap)
				Expect(err).To(MatchError("CryptoInvalidValueLength: PUBS"))
			})
			...
			It("ensure that C255 Pubs must not be at the first index", func() {
				serverKex, err := crypto.NewCurve25519KEX()
				Expect(err).ToNot(HaveOccurred())
				tagMap[protocol.TagKEXS] = []byte("P256C255") // have another KEXS before C255
				// 3 byte len + 1 byte empty + C255
				tagMap[protocol.TagPUBS] = append([]byte{0x01, 0x00, 0x00, 0x00}, append([]byte{0x20, 0x00, 0x00}, serverKex.PublicKey()...)...)
				err = scfg.parseValues(tagMap)
				Expect(err).ToNot(HaveOccurred())
				sharedSecret, err := serverKex.CalculateSharedKey(scfg.kex.PublicKey())
				Expect(err).ToNot(HaveOccurred())
			...
			It("errors if the PUBS is missing", func() {
				delete(tagMap, protocol.TagPUBS)
				err := scfg.parseValues(tagMap)
		...
		Context("EXPY", func() {
			It("parses the expiry date", func() {
				tagMap[protocol.TagEXPY] = []byte{0xdc, 0x89, 0x0e, 0x59, 0, 0, 0, 0} // UNIX Timestamp 0x590e89dc = 1494125020
				err := scfg.parseValues(tagMap)
				Expect(err).ToNot(HaveOccurred())
