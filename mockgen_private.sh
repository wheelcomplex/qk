...
TEMP_DIR=$(mktemp -d)
mkdir -p $TEMP_DIR/src/github.com/wheelcomplex/qk/

# uppercase the name of the interface
INTERFACE_NAME="$(tr '[:lower:]' '[:upper:]' <<< ${4:0:1})${4:1}"

# copy all .go files to a temporary directory
rsync -r --exclude 'vendor' --include='*.go' --include '*/' --exclude '*'   $GOPATH/src/github.com/wheelcomplex/qk/ $TEMP_DIR/src/github.com/wheelcomplex/qk/

# create a public alias for the interface, so that mockgen can process it
echo -e "package $1\n" > $TEMP_DIR/src/github.com/wheelcomplex/qk/mockgen_interface.go
echo "type $INTERFACE_NAME = $4" >> $TEMP_DIR/src/github.com/wheelcomplex/qk/mockgen_interface.go
