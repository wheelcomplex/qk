...
type serverTLS struct {
	...
	newSession func(connection, sessionRunner, protocol.ConnectionID, protocol.ConnectionID, protocol.PacketNumber, *Config, handshake.MintTLS, *handshake.CryptoStreamConn, handshake.AEADWithPacketNumberCrypto, *handshake.TransportParameters, protocol.VersionNumber, utils.Logger) (quicSession, error)
...
func (s *serverTLS) handleInitialImpl(remoteAddr net.Addr, hdr *wire.Header, data []byte) (packetHandler, protocol.ConnectionID, error) {
	...
	aead, err := handshake.NewNullAEAD(hdr.DestConnectionID, protocol.PerspectiveServer)
	if err != nil {
		return nil, nil, err
	}
...
func (s *serverTLS) handleUnpackedInitial(
	...
	aead handshake.AEADWithPacketNumberCrypto,
